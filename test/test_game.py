...
def test_process_client_message():
    ...
    # Act
    message_answer_response_incorrect = game.process_client_message(message_answer_false, game.player_database, 1,
                                                                    board)
    message_answer_response_correct = game.process_client_message(message_answer_true, game.player_database, 1, board)
    message_active_status_response = game.process_client_message(message_active_status, game.player_database, 1, board)
    message_request_board_response = game.process_client_message(message_request_board, game.player_database, 1, board)
    message_disconnect_response = game.process_client_message(message_disconnect, game.player_database, 1, board)
    message_question_response = game.process_client_message(message_question, game.player_database, 1, board)

    # Assert
    assert message_question_response.header.message_type == MessageTypes.QUESTION
    assert message_answer_response_incorrect.header.message_type == MessageTypes.INCORRECT_ANSWER
    assert message_answer_response_incorrect.body["correct_answer"] == question.correct_answer
    assert message_answer_response_correct.header.message_type == MessageTypes.CORRECT_ANSWER
    assert message_active_status_response.header.message_type == MessageTypes.ACTIVE_STATUS
